...
def collate_MIL_survival(batch):
    ...
    case_id = [item[4] for item in batch]
...
def collate_MIL_survival_graph(batch):
    ...
    return [graph_batch, labels, event_time, c, case_id]
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False, mode='coattn', batch_size=1):
    ...
    kwargs = {'num_workers': 4} if device.type == "cuda" else {}#TODO 调试时改为0，否则报错
    if not testing:
        if training:
            if weighted:
