...
class Generic_WSI_Survival_Dataset(Dataset):
    ...
    def get_split_from_df(self, all_splits: dict, split_key: str='train', scaler=None):
        ...
        if len(split) > 0:
            ...
            split_df = pd.DataFrame({"slide_id": split_list})
            split_df["sort_key"] = split_df.index

            df_slice = split_df.merge(
                self.slide_data,
                on="slide_id",
                how="left"
            ).sort_values("sort_key").drop("sort_key", axis=1)
    ...
    def return_splits(self, from_id: bool=True, csv_path: str=None):
        if from_id:
            ...
            val_split = self.get_split_from_df(all_splits=all_splits, split_key='validation')
            ...
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    for slide_id in slide_ids:
                        wsi_path = os.path.join('/root/Desktop/data/private/LIHC/feature_DX_UNI2/pt_files',  '{}.pt'.format(slide_id.rstrip('.svs')))
                    ...
                    for slide_id in slide_ids:
                        wsi_path = os.path.join('/root/Desktop/data/private/LIHC/patch_graph_343_uni2', '{}.pt'.format(slide_id.rstrip('.svs')))
                        wsi_bag = torch.load(wsi_path)
                        path_features.append(wsi_bag)
                ...
                return slide_ids, label, event_time, c, case_id
