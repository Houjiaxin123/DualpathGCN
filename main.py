...
def main(args):
    ...
    for i in folds:
        ...
        results_pkl_path_train = os.path.join(args.results_dir, 'split_latest_train_{}_results.pkl'.format(i))
        results_pkl_path = os.path.join(args.results_dir, 'split_latest_val_{}_results.pkl'.format(i))
        results_pkl_path_test = os.path.join(args.results_dir, 'split_latest_test_{}_results.pkl'.format(i))
        ...
        if args.task_type == 'survival':
            train_latest, cindex_latest_train,val_latest, cindex_latest, test_latest, cindex_latest_test = train(datasets, i, args)
            latest_train_cindex.append(cindex_latest_train)
            latest_val_cindex.append(cindex_latest)
            latest_test_cindex.append(cindex_latest_test)
        ### Write Results for Each Split to PKL
        save_pkl(results_pkl_path_train, train_latest)
        save_pkl(results_pkl_path, val_latest)
        save_pkl(results_pkl_path_test, test_latest)
        end = timer()
        print('Fold %d Time: %f seconds' % (i, end - start))

    ### Finish 5-Fold CV Evaluation.
    if args.task_type == 'survival':
        results_latest_df_train = pd.DataFrame({'folds': folds, 'train_cindex': latest_train_cindex})
        results_latest_df = pd.DataFrame({'folds': folds, 'val_cindex': latest_val_cindex})
        results_latest_df_test = pd.DataFrame({'folds': folds, 'test_cindex': latest_test_cindex})

    if len(folds) != args.k:
        save_name = 'summary_partial_{}_{}.csv'.format(start, end)
    else:
        save_name = 'summary.csv'
    results_latest_df_train.to_csv(os.path.join(args.results_dir, 'summary_latest_train.csv'))
    results_latest_df.to_csv(os.path.join(args.results_dir, 'summary_latest_val.csv'))
    results_latest_df_test.to_csv(os.path.join(args.results_dir, 'summary_latest_test.csv'))
...
parser.add_argument('--data_root_dir', type=str, default='/root/Desktop/data/private/LIHC', help='data directory')
...
parser.add_argument('--results_dir',     type=str, default='/root/Desktop/data/private/hjx_product/results_check', help='Results directory (Default: ./results)')
...
parser.add_argument('--split_dir',       type=str, default='lihc', help='Which cancer type within ./splits/<which_splits> to use for training. Used synonymously for "task" (Default: tcga_blca_100)')
...
### Model Parameters.
parser.add_argument('--model_type',      type=str, choices=['amil', 'patchgcn', 'h2gcn', 'CombinedModel'], default= 'CombinedModel', help='Type of model (Default: mcat)')
parser.add_argument('--mode',            type=str, choices=['path', 'graph'], default='graph', help='Specifies which modalities to use / collate function in dataloader.')
parser.add_argument('--num_gcn_layers',  type=int, default=4, help = '# of GCN layers to use.')
parser.add_argument('--edge_agg',        type=str, choices=['spatial', 'latent', None], default=['latent', 'latent'], help="which graph to use for aggregation. index 0 is for patchgcn, index 1 is for h2gcn")
...
parser.add_argument('--bag_loss',        type=str, choices=['nll_surv'], default='nll_surv', help='slide-level classification loss function (default: ce)')
...
parser.add_argument('--alpha_surv',      type=float, default=0.0, help='How much to weigh uncensored patients (default:0.0)')
...
if 'survival' in args.task:
    ...
    dataset = Generic_MIL_Survival_Dataset(csv_path = '/root/Desktop/data/private/LIHC/tcga_lihc_survival.csv', #'./%s/%s_all_clean.csv.zip' % (args.dataset_path, combined_study),
                                           mode = args.mode,
...
args.split_dir = '/root/Desktop/data/private/LIHC/5_fold/splits/lihc_343' #os.path.join('./splits', args.which_splits, args.split_dir)
